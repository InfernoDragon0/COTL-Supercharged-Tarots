        ...
        public static void PlayerWeapon_DoBlunderbussAttack(PlayerWeapon __instance, Health.AttackTypes attackType, ref bool __result)
        {
            if (TrinketManager.HasTrinket(Plugin.multishotX)) {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    Vector3 Position = __instance.transform.position + new Vector3(__instance.playerFarming.CurrentWeaponInfo.WeaponData.Combos[__instance.CurrentCombo].RangeRadius * __instance.playerFarming.CurrentWeaponInfo.RangeMultiplier * Mathf.Cos(__instance.state.facingAngle * ((float) Math.PI / 180f)), __instance.playerFarming.CurrentWeaponInfo.WeaponData.Combos[__instance.CurrentCombo].RangeRadius * __instance.playerFarming.CurrentWeaponInfo.RangeMultiplier * Mathf.Sin(__instance.state.facingAngle * ((float) Math.PI / 180f)), -0.5f);
                    Health.AttackFlags AttackFlags = 0;
                    /*__instance.additionalBlunderbussFlags = AttackFlags;*/
                    component.destroyAfterDuration = true;
                    component.Init(Position, adjustedFacingAngle, (Health.Team) ((__instance.overrideTeam != null) ? (int)__instance.overrideTeam : (int) __instance.health.team), __instance.health, new Action<Health, Health.AttackTypes, Health.AttackFlags, float>(__instance.BlunderBussEnemyCallBack), __instance.playerFarming.CurrentWeaponInfo.WeaponData.Combos[__instance.CurrentCombo].RangeRadius * __instance.playerFarming.CurrentWeaponInfo.RangeMultiplier, 0.0f, __instance.GetCritChance(), attackType, AttackFlags);
